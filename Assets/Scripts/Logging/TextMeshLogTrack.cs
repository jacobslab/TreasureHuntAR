...
public class TextMeshLogTrack : LogTrack {
	...
	void LateUpdate () {
		...
		if(Configuration.isLogging && ( currentText != myText.text ) ){ //if the text has changed, log it!
			LogText ();
		}
		if(Configuration.isLogging && ( currentColor != myText.color ) ){ //if the text has changed, log it!
			LogColor ();
		}
	}
